<script lang="ts">
	...
	import { createNewProduct } from '$lib/apis/products';
	...
	const handleSubmit = async () => {
		if (!name || price <= 0 || !shop_id || imageFileIds.length === 0) {
		...
		loading = true;
		try {
			const res = await createNewProduct(localStorage.token, {
				name,
	...
	onMount(async () => {
		...
		try {
			const res = await searchShops(localStorage.token, null, 'created', null, null, null, 1);
			if (res && res.items) {
				shops = res.items;
			}
		} catch (error) {
...
<div class="max-w-2xl mx-auto p-6">
	<h1 class="text-2xl font-bold mb-6">{$i18n.t('Create Product')}</h1>
	...
	>
		...
		<div>
			...
			{#if loadingShops}
				<div class="w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100">
					{$i18n.t('Loading shops...')}
				</div>
		...
		<div class="flex gap-4">
			...
			>
				{loading ? $i18n.t('Creating...') : $i18n.t('Create Product')}
			</button>
			<button
				type="button"
