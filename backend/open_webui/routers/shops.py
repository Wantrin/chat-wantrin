...
@router.get("/{id}", response_model=Optional[ShopResponse])
async def get_shop_by_id(
    ...
    shop = Shops.get_shop_by_id(id, db=db)
    if not shop:
        raise HTTPException(
...
@router.post("/{id}/update", response_model=Optional[ShopModel])
async def update_shop_by_id(
    ...
    try:
        shop = Shops.update_shop_by_id(id, form_data, db=db)
        await sio.emit(
            "shop-events",
            shop.model_dump(),
            to=f"shop:{shop.id}",
...
async def delete_shop_by_id(
    ...
    try:
        shop = Shops.delete_shop_by_id(id, db=db)
        return True
    except Exception as e:
        log.exception(e)
        raise HTTPException(
